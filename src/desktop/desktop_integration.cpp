...
bool DesktopIntegration::installDesktop(const DesktopEntry& entry) {
    ...
    if (entry.name.empty() || entry.exec.empty()) {
        SSPM_ERROR("[desktop] name 和 exec 不能为空");
        return false;
    }

    std::string dir = applicationsDir();
    try { fs::create_directories(dir); }
    catch (...) { SSPM_ERROR("[desktop] 无法创建目录: " + dir); return false; }
    ...
    SSPM_INFO("生成 .desktop: " + filePath);

    std::ofstream f(filePath);
    if (!f) {
        SSPM_ERROR("[desktop] 无法写入: " + filePath);
        return false;
    }
    ...
    SSPM_INFO(".desktop 已生成: " + filePath);
    refreshDesktopDb();
    return true;
}
...
bool DesktopIntegration::uninstallDesktop(const std::string& appName) {
    ...
    if (!fs::exists(filePath)) {
        SSPM_WARN("[desktop] 未找到 .desktop 文件: " + filePath);
        return false;
    }

    fs::remove(filePath);
    SSPM_INFO(".desktop 已删除: " + filePath);
    refreshDesktopDb();
    return true;
}
...
bool DesktopIntegration::registerMime(const MimeAssoc& assoc) {
    if (assoc.mimeType.empty() || assoc.appDesktopId.empty()) {
        SSPM_ERROR("[mime] mimeType 和 appDesktopId 不能为空");
        return false;
    }
    ...
    SSPM_INFO("注册 MIME: " + assoc.mimeType + " → " + assoc.appDesktopId);
    ...
    SSPM_INFO("MIME 关联已注册: " + assoc.mimeType);
    refreshMimeDb();
    return true;
}
...
void DesktopIntegration::refreshDesktopDb() {
    // 检查 update-desktop-database 是否存在
    if (system("which update-desktop-database > /dev/null 2>&1") != 0) return;

    // 执行 update-desktop-database
    std::string cmd = "update-desktop-database " + applicationsDir();
    system(cmd.c_str());
}

void DesktopIntegration::refreshMimeDb() {
    // 检查 xdg-mime 是否存在
    if (system("which xdg-mime > /dev/null 2>&1") != 0) return;

    // 执行 xdg-mime
    system("xdg-mime default > /dev/null 2>&1");
}
